...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels: int = 1, patch_size: int = 16, emb_size: int = 256, img_size: int = 256):
        ...
        # self.positions = nn.Parameter(torch.randn(img_size, emb_size))
        # self.lin = nn.Sequential(
        #     Rearrange('b (e) h (w) -> b h (e w)'),
        #     nn.Linear(img_size//2+1, emb_size))
        self.positions = nn.Parameter(torch.randn(img_size//2+1, emb_size))
        self.lin = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x = abs(torch.fft.rfft2(x, dim=(2, 3), norm='ortho')) # b, 1, 256, 256 -> b, 1, 256, 129
        x = self.lin(x)
        x += self.positions
        return x

class Rfft_mymodel(nn.Module):
    def __init__(self, enet_type=None, out_dim=4, img_size=256, patch_size=16, n_classes=4, dim=256, depth=12, heads=12, mlp_dim=256, pool = 'cls', channels = 1, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        # self.to_patch_embedding = PatchEmbedding2(in_channels=channels,patch_size=patch_size,emb_size=dim,img_size=img_size)
