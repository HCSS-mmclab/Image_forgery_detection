...
from dataset import get_dataframe, get_transforms, resamplingDataset, resamplingDataset_modified, resamplingDataset_orgimg
from models import JUNet, MISLnet, CSSNet
...
def parse_args():
    ...
    parser.add_argument('--init-lr', type=float, default=4e-3) # 초기 러닝 레이트. pretrained를 쓰면 매우 작은값
...
def run(fold, df,df_test, transforms_train, transforms_val):
    '''
    학습 진행 메인 함수
    ...
    if args.k_fold == 1:
        df_train = df
        df_valid = df_test
        #k-fold 1일때, fold 대신 미리 준비해둔 validation set 사용
    ...
    # 데이터셋 읽어오기
    dataset_train = resamplingDataset_orgimg(df_train, 'train', args.image_size, transform=transforms_train)
    dataset_valid = resamplingDataset_orgimg(df_valid, 'valid', args.image_size, transform=transforms_val)
    ...
    # model_file = os.path.join(args.model_dir, f'0830_mislnet_modelchanged_4e-3_20ep_best_fold{fold}.pth')
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        if epoch>0:
            val_loss, val_r_loss, val_s_loss, acc_score = val_epoch(model, valid_loader)
        ...
        if epoch == args.n_epochs:
            ...
            # plt.show()
            plt.savefig(f'./plot_img/{args.kernel_type}_fold{fold}.png')
...
def main():
    ...
    folds = [0]
    for fold in folds:
        run(fold, df_train, df_test, transforms_train, transforms_val)
